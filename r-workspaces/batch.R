...
B <- read.csv("data/한국전력거래소_지역별 시간별 태양광 및 풍력 발전량_20230610.csv", 
              header = TRUE, fileEncoding = "CP949")

str(B)

install.packages("data.table")
library(data.table)
C <- fread("data/한국전력거래소_지역별 시간별 태양광 및 풍력 발전량_20230610.csv", encoding = "UTF-8")
head(C)

A = read.csv("data/titanic.csv",header=T)
head(A)
...
remove.packages("party")  # 기존 패키지 제거
install.packages("party")  # 패키지 재설치
library(party)  # 다시 로드
...
# 모델 학습 (예시로 GE_PBA_A_검사서_new 데이터를 사용)
model <- randomForest(ResultOK ~ BLE.RSSI + ATIVECURR + STANBYCURR + IR.Current + IR.LED + ACC_X + ACC_Y + ACC_Z, data = GE_PBA_A_검사서_new)
...
ggplot(importance_data, aes(x = reorder(rownames(importance_data), -`MeanDecreaseGini`), y = `MeanDecreaseGini`)) +
  ...
  coord_flip() +
  labs(title = "Feature Importance", x = "Features", y = "F score") +
  theme_minimal()
